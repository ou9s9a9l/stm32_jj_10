...
TestStatus CAN_Polling(void)
{
  ...
  CAN_FilterInitStructure.CAN_FilterIdHigh=0x0060;
  CAN_FilterInitStructure.CAN_FilterIdLow=0x0000;
  CAN_FilterInitStructure.CAN_FilterMaskIdHigh=0x00f0;
...
TestStatus CAN_Interrupt(void)
{
  ...
  CAN_InitStructure.CAN_RFLM=DISABLE;	          // FIFO没有锁定，新报文覆盖旧报文
  CAN_InitStructure.CAN_TXFP=DISABLE;           // 发送报文优先级确定：标志符
  CAN_InitStructure.CAN_Mode=CAN_Mode_Silent; 	// 回环模式
  CAN_InitStructure.CAN_SJW=CAN_SJW_3tq;        // 1tq、BS1、BS2的值跟波特率有关
  CAN_InitStructure.CAN_BS1=CAN_BS1_10tq;
  CAN_InitStructure.CAN_BS2=CAN_BS2_5tq;
  CAN_InitStructure.CAN_Prescaler=2;					  // 分频系数为1 250kbps
  CAN_Init(CAN1, &CAN_InitStructure);						// 初始化CAN1
  ...
  CAN_FilterInitStructure.CAN_FilterMode=CAN_FilterMode_IdMask;
  CAN_FilterInitStructure.CAN_FilterScale=CAN_FilterScale_32bit;
  CAN_FilterInitStructure.CAN_FilterIdHigh=0x0b60;
  CAN_FilterInitStructure.CAN_FilterIdLow=0x0000;
  CAN_FilterInitStructure.CAN_FilterMaskIdHigh=0x00ff;//0xFFFF;
...
/*  TxMessage.StdId=0x00;					 // 标准标识符为0
  ...
  TxMessage.Data[1]=0xCA;				 // 第二帧信息
	TxMessage.StdId=0x07ff;				// 设定标准标识符（11位，扩展的为29位）右移5位 <<11
  TxMessage.RTR=CAN_RTR_DATA;	// 传输消息的帧类型为数据帧（还有远程帧）
  TxMessage.IDE=CAN_ID_STD;		// 消息标志符实验标准标识符
  TxMessage.DLC=2;					  // 发送两帧，一帧8位
