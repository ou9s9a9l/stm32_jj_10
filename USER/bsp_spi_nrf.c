...
//#include "bsp_usart1.h"
 ...
 u8 TX_ADDRESS[TX_ADR_WIDTH] = {0X11,0X22,0X33,0X44,0X58};  // 定义一个静态发送地址
...
void SPI_NRF_Init(void)
{
 ...
 /*使能SPI1时钟*/
  ...
  GPIO_Init(GPIOA, &GPIO_InitStructure);  */
	/*配置 SPI_NRF_SPI的 SCK,,MOSI引脚，GPIOA^5,GPIOA^6,GPIOA^7 */
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_14|GPIO_Pin_15;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_10MHz;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
  GPIO_Init(GPIOB, &GPIO_InitStructure);
	
/*配置 SPI_NRF_SPI的 MISO引脚，GPIOA^5,GPIOA^6,GPIOA^7 */
...

{
   ...
   SPI_NRF_WriteReg(NRF_WRITE_REG+RF_SETUP,0x27);  //设置TX发射参数,0db增益,//250kbps 2Mbps,低噪声增益开启   
	
   SPI_NRF_WriteReg(NRF_WRITE_REG+CONFIG,0x0f);    //配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,发射模式,开启所有中断
...
u8 NRF_Rx_Dat(u8 *rxbuf)
{
	...
	for(  i=0;i<32;i++)rxbuf[i]=0;
	  ...
	  SPI_NRF_ReadBuf(RD_RX_PLOAD,rxbuf,temp);//读取数据
